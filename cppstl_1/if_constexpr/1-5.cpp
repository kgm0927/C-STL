...
// 예제 구현
// 
// 이 헬퍼 템플릿 클래슨느 다양한 템플릿 타입 특수화를 다룰 수 있는데, 어떤 타입을 특수화하는지에 따라
...
// 매우 복잡해 보인다. 그리고 조건식이 true이면 타입을 평가하는 std::enable_if_t<condition,typ>과
...
// 예기다. 이 경우 해당 함수 중 하나의 반환값이 잘못된 템플릿 표현식으로부터 추론할 수 없더라도(enable_if에서 false가 될 때)
